...
默认只部署 MCSManager 管理端口，并绑定在 NUC 的所有网卡上，供局域网通过 NUC LAN IP 访问。游戏端口由你在 MCSManager 中手动创建实例时配置：
...
建议在路由器里给 NUC 做 DHCP 静态绑定，例如固定成：
...
```text
Web:    http://192.168.1.50:23333
Daemon: 192.168.1.50:24444
```
...
在 MCSManager 里创建 Docker 实例时，如果局域网玩家要直连，宿主机端口绑定到 `0.0.0.0` 或 NUC 的局域网 IP，例如：
...
如果只想通过 Sakura Frp 暴露实例端口，再把实例端口绑定到 `127.0.0.1`。
